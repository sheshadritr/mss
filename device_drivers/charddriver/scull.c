...
ssize_t scull_write(struct file *fp, const char __user *buf, size_t count, loff_t *offp)
{
	...
	{
		temp = kmalloc((strlen(dev.data)+1), GFP_KERNEL);
		memcpy(temp, dev.data, (strlen(dev.data)+1));
		kfree((void *)dev.data);
		dev.data = NULL;

		len = *offp + count + 2;
		dev.data = kmalloc(len, GFP_KERNEL);

		memcpy(dev.data, temp, *offp);
		ret = copy_from_user(dev.data + *offp, buf, count);

		dev.data[*offp + count + 1] = '\0';
	}
